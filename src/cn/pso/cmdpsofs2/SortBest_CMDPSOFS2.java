...
public class SortBest_CMDPSOFS2 {

	public void getCMDResult(String dataSetName,List<Particle_CMDPSOFS2> list,String type) throws IOException {
		String path1 = "testfile\\"+dataSetName;
		File f = new File(path1);
		if(!f.exists()){
			f.mkdir();
		}
		String path = path1+"\\"+type+dataSetName+"-CMD2-result.dat";
		...
		List<Particle_CMDPSOFS2> nonDomS = new ArrayList<Particle_CMDPSOFS2>();
	   	 for (int i = 0; i < list.size();i++){
	     	int index = -1; 
		...
		for(Particle_CMDPSOFS2 p:nonDomS){
			System.out.println(p.pbest_fitness[0]+"-------"+p.pbest_fitness[1]);
//			System.out.println(p.pbest_pbest_fitness[0]+"------"+p.pbest_pbest_fitness[1]);
		...
		pw.close();
		System.out.println(dataSetName+":CMD2done!");
	}
}
